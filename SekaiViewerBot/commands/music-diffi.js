...
module.exports = class MusicDataCommand extends Command {
  constructor() {
    super('music-diffi', {
      aliases: ['music-difficulty', 'song-difficulty', 'music-diffi', 'song-diffi', 'difficulty', 'diffi'],
      channel: 'guild',
      ownerOnly: false,
      args: [
        {
  ...
  exec(message, args) {
    ...
    Util.fetchData(
      ...
      if (!data) return;
      const diffis = data.filter(elem => elem.musicId === songId)

      const embed = new Discord.MessageEmbed()
        .setTitle(`Song Difficulty: ${songName}`)
      ...
      diffis.forEach(elem => embed.addField(`difficulty: **${elem.musicDifficulty}**`, `level: **${elem.playLevel}** | notes: **${elem.noteCount}**`))
